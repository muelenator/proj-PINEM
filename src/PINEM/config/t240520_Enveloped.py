...
if not config['Constant coupling']:
    ...
    if config['Enforce phase']:
        ...
        for i, l in enumerate(Lfree):
            ...
            # First cavity
            gQ[:,i,0] = e_c * vs * Ez_ints1 / 2 / hbar_Js / wg['omega']
            # Second cavity
            gQ[:,i,1] = e_c * vs * Ez_ints2[:,i] / 2 / hbar_Js / wg['omega']
    else:
        for i, l in enumerate(Lfree):
            # Both cavities
            gQ[:,i,:] = e_c * vs * Ez_ints1 / 2 / hbar_Js / wg['omega']
